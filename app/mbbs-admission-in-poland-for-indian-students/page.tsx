...
import PolandLeadSection from "./PolandLeadSection";
import {
  additionalCosts,
  advantages,
  articleSchema,
    ...
    <main className="bg-[linear-gradient(180deg,#eff6ff_0%,#f8fafc_28%,#ffffff_65%)] text-slate-900">
      ...
      <section className="relative overflow-hidden border-b border-sky-100">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-20">
          <div>
            <div className="inline-flex rounded-full border border-sky-200 bg-white/90 px-4 py-2 text-sm font-medium text-sky-800 shadow-sm">2026-27 Poland admissions guide for Indian medical aspirants</div>
            ...
            <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-700 md:text-xl">Compare entrance-test effort, total cost, FMGE context, Schengen mobility, university choice depth and India-return practicality before committing to the Poland route.</p>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a href="#poland-contact-form" className="rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-500 hover:text-slate-950">Request Callback</a>
      ...
      <section id="quick-summary" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Quick Summary" title="A fast Poland snapshot before you go deeper" />
        ...
        <div className="mt-8 rounded-[28px] border border-sky-200 bg-sky-50 p-5 text-sm leading-7 text-sky-950">Poland is one of the stronger EU medicine routes for Indian students who want more university options than Lithuania and lower cost pressure than Hungary.</div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        <SectionHeading eyebrow="Key Facts" title="At-a-glance Poland medicine facts for 2026-27" />
        <div className="mt-10"><DataTable rows={keyFacts} caption="Poland medicine key facts table" /></div>
      </section>

      <section id="timeline" className="bg-slate-950 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Timeline" title="Admission planning for the Poland route" theme="dark" />
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        <div className="mt-10"><DataTable rows={eligibility} caption="Poland medicine eligibility table" /></div>
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {eligibilityNotes.map((note) => (
            <div key={note} className="rounded-3xl border border-slate-200 bg-slate-50 p-5 text-sm leading-7 text-slate-700">
                {note.includes("/mbbs-admission-in-bulgaria-for-indian-students") ? (
                <>
                    If you want a lower-cost EU alternative, compare Poland with <Link href="/mbbs-admission-in-bulgaria-for-indian-students" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Bulgaria for Indian students</Link> and <Link href="/mbbs-admission-in-lithuania" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Lithuania 2026-27</Link> before deciding.
                </>
              ) : (
                note
              )}
      ...
      <section id="universities" className="bg-slate-50 py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Top Universities" title="Poland options students most often compare" />
          <div className="mt-10"><DataTable rows={universities} caption="Poland top universities table" /></div>
        </div>
      </section>

      <section id="fees" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Fees Breakdown" title="Transparent budgeting before you commit to Poland" />
        <div className="mt-10"><DataTable rows={feeBreakdown} caption="Poland fee breakdown table" /></div>
        <h3 className="mt-10 text-xl font-bold text-slate-900">Common additional costs students should budget for</h3>
        <div className="mt-5"><DataTable rows={additionalCosts} caption="Poland additional cost table" /></div>
      </section>

      <section id="exam-context" className="bg-[linear-gradient(180deg,#f8fafc_0%,#eff6ff_100%)] py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="FMGE / NExT Context" title="India licensing context for Poland graduates" />
          <div className="mt-10 grid gap-8 xl:grid-cols-2">
            <div>
              <h3 className="mb-4 text-xl font-bold text-slate-900">Recent Poland comparison context</h3>
              <DataTable rows={examContext} caption="Poland exam context table" />
            </div>
            <div>
              <h3 className="mb-4 text-xl font-bold text-slate-900">What those numbers mean in practice</h3>
              <DataTable rows={polandExamNotes} caption="Poland exam notes table" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        <div className="mt-10"><DataTable rows={recognition} caption="Poland recognition table" /></div>
      </section>

      <section className="bg-slate-950 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Curriculum" title="Year-wise curriculum shape in Poland medicine" theme="dark" />
          <div className="mt-10"><DataTable rows={syllabus} caption="Poland medicine syllabus table" /></div>
        </div>
      </section>
      ...
      <section className="bg-slate-50 py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Living Costs" title="Monthly living picture in Poland" />
          <div className="mt-10"><DataTable rows={livingCosts} caption="Poland living cost table" /></div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Pros And Cons" title="Balanced view before you choose Poland" />
      ...
      <section className="bg-[linear-gradient(180deg,#ffffff_0%,#eff6ff_100%)] py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Alternatives" title="How Poland compares with other common destinations" />
          <div className="mt-10"><DataTable rows={comparison} caption="Poland alternatives comparison table" /></div>
          <p className="mt-6 text-sm leading-7 text-slate-600">
            Compare Poland with <Link href="/mbbs-admission-in-lithuania" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Lithuania 2026-27</Link>, <Link href="/mbbs-in-hungary" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Hungary 2026-27 for Indian students</Link>, <Link href="/mbbs-admission-in-bulgaria-for-indian-students" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Bulgaria for Indian students</Link> and the current <Link href="/contact?interest=mbbs-in-georgia" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Georgia 2026</Link> guidance path. For India licensing planning, review <Link href="/contact?interest=nmc-next-exam-guide" className="font-semibold text-sky-700 underline underline-offset-4">NMC NEXT exam preparation guide</Link>, <Link href="/mbbs-without-neet" className="font-semibold text-sky-700 underline underline-offset-4">MBBS without NEET for Indian students</Link>, and <Link href="/contact?interest=cheapest-mbbs-in-europe" className="font-semibold text-sky-700 underline underline-offset-4">cheapest MBBS in Europe for Indian students</Link>.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        <div className="mt-10"><DataTable rows={scholarships} caption="Poland scholarships table" /></div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        <div className="mt-10"><DataTable rows={careerPathways} caption="Poland career pathways table" /></div>
      ...
      <section id="faq" className="bg-slate-950 py-16 text-white">
        <div className="mx-auto max-w-5xl px-4 lg:px-8">
          <SectionHeading eyebrow="FAQ" title="Common Poland medicine questions Indian students ask" theme="dark" />
