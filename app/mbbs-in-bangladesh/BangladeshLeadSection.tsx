...
function BangladeshLeadForm({
  ...
  const universityOptions = useMemo(() => universities.map((item) => item.College), []);

  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label htmlFor="bangladesh-name" className="mb-1 block text-sm font-medium text-slate-700">
          Full name
        </label>
        <input
          id="bangladesh-name"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label htmlFor="bangladesh-phone" className="mb-1 block text-sm font-medium text-slate-700">
            WhatsApp number
          </label>
          <input
            id="bangladesh-phone"
        ...
        <div>
          <label htmlFor="bangladesh-email" className="mb-1 block text-sm font-medium text-slate-700">
            Email
          </label>
          <input
            id="bangladesh-email"
      ...
      <div>
        <label htmlFor="bangladesh-university" className="mb-1 block text-sm font-medium text-slate-700">
          Preferred college
        </label>
        <select
          id="bangladesh-university"
...
export default function BangladeshLeadSection() {
  ...
  const submitLead = async (
    ...
    const requestData = {
      fields: {
        ...
        Course: "MBBS Abroad",
        StudyCountry: "Bangladesh",
        College: activeForm.preferredUniversity,
      },
      actions: [
        {
          type: "SYSTEM_NOTE",
          text: "Lead Source: MBBS in Bangladesh Page",
        },
      ],
    };
    ...
    try {
      ...
      router.push("/thank-you?title=MBBS%20in%20Bangladesh");
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      {showPopup ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[32px] bg-white p-6 shadow-2xl md:p-8">
            ...
            <h3 className="mt-3 text-3xl font-bold text-slate-950">Get personalised Bangladesh admission guidance</h3>
            <p className="mt-3 text-base leading-7 text-slate-600">Share your details and our team can help with college shortlisting, fee comparison, NEET eligibility, visa steps and Bangladesh living expectations.</p>
      ...
      <section id="bangladesh-contact-form" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[32px] bg-[linear-gradient(135deg,#1d4ed8_0%,#0f172a_100%)] p-8 text-white">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-200">Contact Bangladesh Desk</p>
            <h2 className="mt-4 text-3xl font-bold">Speak to our team on WhatsApp or request a callback</h2>
            <p className="mt-4 text-base leading-8 text-slate-300">Use this section for college comparisons, fee planning, scholarship questions, visa support and Bangladesh 2026-27 intake guidance.</p>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
            ...
            <p className="mt-3 text-base leading-7 text-slate-600">Fill this once and the team can contact you with Bangladesh options that fit your NEET score, budget and India-return goals.</p>
            <div className="mt-8">
              <BangladeshLeadForm
                formData={formData}
