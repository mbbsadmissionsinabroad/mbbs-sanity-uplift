...
import { ivrNumber, levelsOverview, whatsappHref, whatsappNumber } from "./pageData";
...
function LearnGermanForm({
  ...
  const levelOptions = useMemo(() => levelsOverview.map((item) => `${item.Level} - ${item.Name}`), []);

  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label htmlFor="german-name" className="mb-1 block text-sm font-medium text-slate-700">
          Full name
        </label>
        <input
          id="german-name"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label htmlFor="german-phone" className="mb-1 block text-sm font-medium text-slate-700">
            WhatsApp number
          </label>
          <input
            id="german-phone"
        ...
        <div>
          <label htmlFor="german-email" className="mb-1 block text-sm font-medium text-slate-700">
            Email
          </label>
          <input
            id="german-email"
      ...
      <div>
        <label htmlFor="german-level" className="mb-1 block text-sm font-medium text-slate-700">
          Preferred level
        </label>
        <select
          id="german-level"
...
export default function LearnGermanLeadSection() {
  ...
  const submitLead = async (
    ...
    const requestData = {
      fields: {
        ...
        College: activeForm.preferredLevel,
      },
      actions: [
        {
          type: "SYSTEM_NOTE",
          text: "Lead Source: Learn German Bangalore Page",
        },
      ],
    };
    ...
    try {
      ...
      router.push("/thank-you?title=German%20Language%20Course");
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      {showPopup ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[32px] bg-white p-6 shadow-2xl md:p-8">
            ...
            <h3 className="mt-3 text-3xl font-bold text-slate-950">Get personalised German-course guidance</h3>
            <p className="mt-3 text-base leading-7 text-slate-600">Share your level, goal and timeline, and our team can guide you on the right German batch for study abroad, Ausbildung, nursing, MNC work or visa preparation.</p>
      ...
      <section id="learn-german-contact-form" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[32px] bg-[linear-gradient(135deg,#1d4ed8_0%,#0f172a_100%)] p-8 text-white">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-200">Contact Learn German Desk</p>
            <h2 className="mt-4 text-3xl font-bold">Speak to our team on WhatsApp or request a callback</h2>
            <p className="mt-4 text-base leading-8 text-slate-300">Use this section for level assessment, batch planning, Goethe exam preparation and Germany-linked language guidance.</p>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
            ...
            <h3 className="mt-4 text-3xl font-bold text-slate-950">Request German-course support</h3>
            <p className="mt-3 text-base leading-7 text-slate-600">Fill this once and the team can contact you with the right level, schedule and path for your Germany goal.</p>
            <div className="mt-8">
              <LearnGermanForm
                formData={formData}
