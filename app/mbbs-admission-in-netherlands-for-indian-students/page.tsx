...
import NetherlandsLeadSection from "./NetherlandsLeadSection";
import {
  additionalCosts,
  advantages,
  articleSchema,
    ...
    <main className="bg-[linear-gradient(180deg,#eff6ff_0%,#f8fafc_28%,#ffffff_65%)] text-slate-900">
      ...
      <section className="relative overflow-hidden border-b border-sky-100">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-20">
          <div>
            <div className="inline-flex rounded-full border border-sky-200 bg-white/90 px-4 py-2 text-sm font-medium text-sky-800 shadow-sm">2026-27 Netherlands admissions guide for Indian medical aspirants</div>
            ...
            <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-700 md:text-xl">Compare Dutch language requirements, total cost, EU career value, visa process and India-return practicality before committing to the Netherlands route.</p>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a href="#netherlands-contact-form" className="rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-500 hover:text-slate-950">Request Callback</a>
          ...
          <aside className="self-start rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_30px_80px_rgba(15,23,42,0.08)] lg:sticky lg:top-28">
            ...
            <div className="mt-5 space-y-3">
              {[
                ...
                ["#fees", "Fees breakdown"],
                ["#exam-context", "India-return context"],
                ["#faq", "FAQ section"],
              ].map(([href, label]) => (
      ...
      <section id="quick-summary" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Quick Summary" title="A fast Netherlands snapshot before you go deeper" />
        ...
        <div className="mt-8 rounded-[28px] border border-sky-200 bg-sky-50 p-5 text-sm leading-7 text-sky-950">The Netherlands works best as a premium, language-intensive EU-career route, not as a simple English-medium MBBS abroad option.</div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        <SectionHeading eyebrow="Key Facts" title="At-a-glance Netherlands medicine facts for 2026-27" />
        <div className="mt-10"><DataTable rows={keyFacts} caption="Netherlands medicine key facts table" /></div>
      </section>

      <section id="timeline" className="bg-slate-950 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Timeline" title="Admission planning for the Dutch route" theme="dark" />
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Eligibility" title="Minimum criteria and the real practical filters" />
        <div className="mt-10"><DataTable rows={eligibility} caption="Netherlands medicine eligibility table" /></div>
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {eligibilityNotes.map((note) => (
            <div key={note} className="rounded-3xl border border-slate-200 bg-slate-50 p-5 text-sm leading-7 text-slate-700">
              {note.includes("MBBS in Belarus") ? (
                <>
                    If your main goal is a simpler English-medium route, compare the Netherlands with <Link href="/mbbs-in-belarus-for-indian-students" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Belarus for Indian students</Link> and <Link href="/mbbs-in-georgia" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Georgia for Indian students</Link> before deciding.
                </>
              ) : (
                note
              )}
      ...
      <section id="universities" className="bg-slate-50 py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Top Universities" title="Dutch medicine options students most often compare" />
          <div className="mt-10"><DataTable rows={universities} caption="Netherlands top universities table" /></div>
        </div>
      </section>

      <section id="fees" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Fees Breakdown" title="Transparent budgeting before you commit to the Netherlands" />
        <div className="mt-10"><DataTable rows={feeBreakdown} caption="Netherlands fee breakdown table" /></div>
        <h3 className="mt-10 text-xl font-bold text-slate-900">Common additional costs students should budget for</h3>
        <div className="mt-5"><DataTable rows={additionalCosts} caption="Netherlands additional cost table" /></div>
      </section>

      <section id="exam-context" className="bg-[linear-gradient(180deg,#f8fafc_0%,#eff6ff_100%)] py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="India-Return Context" title="How this route compares for Indian licensing strategy" />
          <div className="mt-10 grid gap-8 xl:grid-cols-2">
            <div>
              <h3 className="mb-4 text-xl font-bold text-slate-900">Recent Netherlands comparison context</h3>
              <DataTable rows={examContext} caption="Netherlands exam context table" />
            </div>
            <div>
              <h3 className="mb-4 text-xl font-bold text-slate-900">What those numbers mean in practice</h3>
              <DataTable rows={netherlandsExamNotes} caption="Netherlands exam notes table" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Recognition" title="Recognition stack and why the Dutch degree stands out" />
        <div className="mt-10"><DataTable rows={recognition} caption="Netherlands recognition table" /></div>
      </section>

      <section className="bg-slate-950 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Curriculum" title="Year-wise curriculum shape in Dutch medicine" theme="dark" />
          <div className="mt-10"><DataTable rows={syllabus} caption="Netherlands medicine syllabus table" /></div>
        </div>
      </section>
      ...
      <section className="bg-slate-50 py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Living Costs" title="Monthly living picture in the Netherlands" />
          <div className="mt-10"><DataTable rows={livingCosts} caption="Netherlands living cost table" /></div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Pros And Cons" title="Balanced view before you choose the Netherlands" />
      ...
      <section className="bg-[linear-gradient(180deg,#ffffff_0%,#eff6ff_100%)] py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Alternatives" title="How the Netherlands compares with other common destinations" />
          <div className="mt-10"><DataTable rows={comparison} caption="Netherlands alternatives comparison table" /></div>
          <p className="mt-6 text-sm leading-7 text-slate-600">
            Compare the Netherlands with <Link href="/mbbs-in-belarus-for-indian-students" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Belarus for Indian students</Link>, <Link href="/mbbs-in-georgia" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Georgia for Indian students</Link>, <Link href="/contact?interest=mbbs-in-germany" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Germany for free</Link>, <Link href="/mbbs-without-neet" className="font-semibold text-sky-700 underline underline-offset-4">MBBS without NEET for Indian students</Link>, <Link href="/contact?interest=bsc-nursing" className="font-semibold text-sky-700 underline underline-offset-4">BSc Nursing abroad</Link> and <Link href="/mbbs-in-uzbekistan" className="font-semibold text-sky-700 underline underline-offset-4">MBBS in Uzbekistan 2026</Link>.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        <div className="mt-10"><DataTable rows={scholarships} caption="Netherlands scholarships table" /></div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        ...
        <div className="mt-10"><DataTable rows={careerPathways} caption="Netherlands career pathways table" /></div>
      </section>
      <StaticPageResourceLinks currentRoute={pageUrl} />


      <NetherlandsLeadSection />

      <section id="faq" className="bg-slate-950 py-16 text-white">
        <div className="mx-auto max-w-5xl px-4 lg:px-8">
          <SectionHeading eyebrow="FAQ" title="Common Netherlands medicine questions Indian students ask" theme="dark" />
