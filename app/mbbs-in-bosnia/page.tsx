...
import BosniaLeadSection from "./BosniaLeadSection";
import {
  advantages,
  articleSchema,
  careerPathways,
...
export default function BosniaPage() {
  return (
    <main className="bg-[linear-gradient(180deg,#f4fbff_0%,#fffaf3_28%,#ffffff_65%)] text-slate-900">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(articleSchema) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }} />

      <section className="relative overflow-hidden border-b border-sky-100">
        <div className="absolute inset-x-0 top-0 h-[34rem] bg-[radial-gradient(circle_at_top_left,_rgba(14,165,233,0.15),_transparent_35%),radial-gradient(circle_at_top_right,_rgba(245,158,11,0.15),_transparent_30%),linear-gradient(180deg,_rgba(255,255,255,0.96),_rgba(255,255,255,0.86))]" />
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-20">
          <div>
            <div className="inline-flex rounded-full border border-sky-200 bg-white/90 px-4 py-2 text-sm font-medium text-sky-800 shadow-sm">
              2026-27 Bosnia admissions guide for Indian medical aspirants
            </div>
            ...
            <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-700 md:text-xl">
              Compare Bosnian universities, fees in INR, NEET rules, European recognition, student life and practical post-graduation pathways before you choose Bosnia for MBBS abroad.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a href="#bosnia-contact-form" className="rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-500 hover:text-slate-950">Request Callback</a>
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2">
              {highlights.map((item) => (
                <div key={item} className="rounded-3xl border border-white/80 bg-white/90 p-5 shadow-[0_10px_35px_rgba(15,23,42,0.07)] backdrop-blur">
                  <p className="text-sm font-semibold uppercase tracking-[0.22em] text-amber-600">Key reason</p>
      ...
      <section id="quick-summary" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading level="h2" eyebrow="Quick Summary" title="A fast Bosnia snapshot before you go deeper" />
        ...
        <div className="mt-8 rounded-[28px] border border-amber-200 bg-amber-50 p-5 text-sm leading-7 text-amber-900">
          Bosnia and Herzegovina is a sovereign European country in the Balkans. Major medical universities are found in Sarajevo, Banja Luka, Tuzla, Mostar and Foca.
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        <SectionHeading eyebrow="Key Facts" title="At-a-glance Bosnia MBBS facts for 2026-27" />
        <div className="mt-10"><DataTable rows={keyFacts} /></div>
      </section>

      <section id="timeline" className="bg-slate-950 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Timeline" title="Admission calendar for the 2026-27 Bosnia intake" theme="dark" />
      ...
      <section id="universities" className="bg-slate-50 py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Universities" title="Top Bosnia medical universities for Indian students" />
          <div className="mt-10"><DataTable rows={universities} /></div>
        </div>
      </section>

      <section id="fees" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Fees Breakdown" title="Year-wise fee picture for major Bosnia universities" />
        ...
        <p className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm leading-7 text-amber-900">
          Hostel estimates usually assume twin sharing, and total budget ranges also include one-time setup costs such as travel, visa, insurance and arrival setup.
        </p>
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <a href="#bosnia-contact-form" className="rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Get a Custom Fee Plan</a>
          <a href={`tel:${ivrNumber.replace(/\s+/g, "")}`} className="rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-500 hover:text-slate-950">Call IVR</a>
        </div>
      </section>

      <section id="fmge" className="bg-[linear-gradient(180deg,#f8fafc_0%,#eef8ff_100%)] py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="FMGE and NEXT" title="Bosnia exam context and what it means" />
          <div className="mt-10 grid gap-8 xl:grid-cols-[0.9fr_1.1fr]">
            <div>
              <h3 className="mb-4 text-xl font-bold text-slate-900">FMGE 2024 context</h3>
            ...
            <div className="rounded-[30px] border border-slate-200 bg-white p-8 shadow-[0_18px_40px_rgba(15,23,42,0.05)]">
              <h3 className="text-xl font-bold text-slate-900">What Bosnia-specific data means today</h3>
              <div className="mt-5 space-y-4 text-base leading-8 text-slate-700">
                <p>Bosnia is still an emerging MBBS destination, so published country-level FMGE data is not as mature as it is for older destinations such as Russia or Bangladesh.</p>
                <p>That makes university selection and early coaching support even more important. Ask each university how they support India-facing students with NEXT or FMGE preparation before paying fees.</p>
                <p>The practical benchmark to compare against is the overall 2024 FMGE context, where coached cohorts from credible foreign universities often perform better than unstructured ones.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Recognition" title="How Bosnia degree recognition affects your career options" />
        <div className="mt-10"><DataTable rows={recognition} /></div>
      </section>

      <section className="bg-slate-50 py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Curriculum" title="Year-wise MBBS syllabus structure in Bosnia" />
          <div className="mt-10"><DataTable rows={syllabus} /></div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="After Graduation" title="Licensing steps to practise in India after MBBS in Bosnia" />
      ...
      <section className="bg-slate-950 py-16 text-white">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Student Life" title="What living in Bosnia usually costs each month" theme="dark" />
          <div className="mt-10"><DataTable rows={livingCosts} /></div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Pros and Cons" title="A realistic view of Bosnia as an MBBS destination" />
      ...
      <section className="bg-slate-50 py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Comparison" title="How Bosnia compares with other MBBS abroad options" />
          <div className="mt-10"><DataTable rows={comparison} /></div>
          <p className="mt-6 text-base leading-7 text-slate-700">
            You can also explore <Link href="/mbbs-in-georgia" className="font-semibold text-sky-700 hover:text-sky-900">MBBS in Georgia for Indian students</Link>,{" "}
            <Link href="/mbbs-in-uzbekistan" className="font-semibold text-sky-700 hover:text-sky-900">MBBS in Uzbekistan 2026</Link>,{" "}
            <Link href="/mbbs-without-neet" className="font-semibold text-sky-700 hover:text-sky-900">MBBS without NEET for Indian students</Link> and{" "}
            <Link href="/bsc-nursing" className="font-semibold text-sky-700 hover:text-sky-900">BSc Nursing abroad</Link>.
          </p>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Funding" title="Scholarships, financial aid and practical funding routes" />
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="Career Pathways" title="What you can do after a Bosnia MBBS degree" />
      ...
      <section className="bg-slate-50 py-12">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 lg:px-8">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-sky-700">Need direct guidance?</p>
            <p className="mt-2 text-lg font-semibold text-slate-900">Talk to the Bosnia team before you shortlist a university.</p>
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <a href="#bosnia-contact-form" className="rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Open Contact Form</a>
      ...
      <section className="bg-slate-50 py-16">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <SectionHeading eyebrow="Without NEET?" title="Why that route does not solve the India-licensing question" description="Students sometimes search for no-NEET options, but NEET remains central if the long-term plan includes India practice." />
          <p className="mt-8 max-w-4xl text-base leading-8 text-slate-700 md:text-lg">
            ...
            <Link href="/mbbs-without-neet" className="font-semibold text-sky-700 hover:text-sky-900">MBBS without NEET for Indian students</Link>. Bosnia may be affordable and globally portable, but India-side licensing still starts with the correct eligibility path.
          </p>
        </div>
      </section>

      <section id="faq" className="mx-auto max-w-5xl px-4 py-16 lg:px-8">
        <SectionHeading eyebrow="FAQ" title="15 common Bosnia questions students and parents ask" />
