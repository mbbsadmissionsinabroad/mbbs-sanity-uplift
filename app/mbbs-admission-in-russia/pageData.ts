export const siteUrl = "https://www.mbbsadmissionsinabroad.com";
export const pageUrl = `${siteUrl}/mbbs-in-russia`;
export const pageTitle =
  "MBBS in Russia 2026 - Complete Guide for Indian Students";
export const metaTitle =
  "MBBS in Russia 2026 | NMC Approved Universities, Fees & Admission Guide";
export const metaDescription =
  "MBBS in Russia 2026 for Indian students. 28 NMC-approved universities compared, fees Rs 21-50 lakhs, FMGE pass rate data, 8-step admission process. Free counselling available.";

export const lastUpdated = "March 2026";
export const whatsappNumber = "+918147030030";
export const whatsappHref =
  "https://api.whatsapp.com/send?phone=918147030030&text=Hi%20I%20want%20guidance%20for%20MBBS%20admission%20in%20Russia%20for%202026-27.";
export const ivrNumber = "+918050575767";

export const quickSummary = [
  {
    feature: "Russia at a glance",
    details:
      "Russia has 55+ NMC-approved medical universities for Indian students.",
  },
  {
    feature: "Annual tuition range",
    details: "Most mainstream MBBS options fall in the Rs 2.5 lakh-Rs 5.5 lakh per year range.",
  },
  {
    feature: "Intake window",
    details: "The 2026-27 intake usually begins in September-October 2026.",
  },
  {
    feature: "NEET rule",
    details: "NEET is mandatory for Indian students under current NMC guidelines.",
  },
  {
    feature: "FMGE context",
    details: "Russia's overall FMGE pass rate is about 29.5% in the 2024 data.",
  },
];

export const highlights = [
  "NMC and WHO-approved universities with globally recognised degrees.",
  "English-medium programs are available at most top Russian medical universities.",
  "No donation or management quota; the standard route is documentation-based and merit-oriented.",
  "The degree is a 6-year structure with academic study plus clinical internship.",
  "Living costs in regional Russian cities can stay around Rs 15,000-Rs 20,000 per month.",
  "There are 55+ NMC-approved Russian universities, so families can compare by outcome as well as by fees.",
  "Indian food, Indian student communities, and Indian mess support exist in many established student cities.",
];

export const keyFacts = [
  { Feature: "Duration", Details: "6 years (5 years academic + 1 year internship)" },
  { Feature: "Intake / Session Start", Details: "September-October 2026" },
  { Feature: "Medium of Instruction", Details: "English, with basic Russian for patient interaction" },
  { Feature: "NEET Required", Details: "Yes, mandatory as per NMC guidelines" },
  { Feature: "Minimum Marks in PCB (12th)", Details: "50% (General) / 45% (SC/ST/OBC)" },
  { Feature: "Minimum Age", Details: "17 years as on December 31 of admission year" },
  { Feature: "Annual Tuition Fees", Details: "Rs 2.5 lakh-Rs 5.5 lakh per year (USD 3,000-7,000)" },
  { Feature: "Hostel Fees", Details: "Rs 50,000-Rs 1 lakh per year" },
  { Feature: "Total Cost (6 Years)", Details: "Rs 20 lakh-Rs 38 lakh all inclusive" },
  { Feature: "Recognition", Details: "NMC, WHO, FAIMER, ECFMG, WFME" },
  { Feature: "Top University", Details: "Kazan State Medical University" },
  { Feature: "FMGE Pass Rate (2024)", Details: "About 29.5% overall; top universities often 40-50%" },
];

export const timeline = [
  { Stage: "Start research and shortlist universities", Timeline: "November-December 2025" },
  { Stage: "Application opens / registration", Timeline: "January-March 2026" },
  { Stage: "Offer letter / invitation letter issued", Timeline: "March-April 2026" },
  { Stage: "NEET scorecard submission and final admission confirmation", Timeline: "May-June 2026" },
  { Stage: "Student visa application and processing", Timeline: "June-August 2026" },
  { Stage: "Travel to Russia", Timeline: "August 2026" },
  { Stage: "Academic session begins", Timeline: "September-October 2026" },
];

export const processSteps = [
  {
    title: "Step 1 - Check your eligibility",
    detail:
      "Confirm you meet the minimum requirements: 50% marks in PCB in Class 12, NEET qualified, age 17+, and a valid passport.",
  },
  {
    title: "Step 2 - Shortlist NMC-approved universities",
    detail:
      "Apply only to universities listed on the current NMC-approved route and compare 3-5 options by fees, location, language structure, and FMGE reputation.",
  },
  {
    title: "Step 3 - Submit the online application",
    detail:
      "Fill in the university application form and upload Class 10 and 12 mark sheets, passport copy, passport-size photos, and NEET scorecard.",
  },
  {
    title: "Step 4 - Receive the invitation letter",
    detail:
      "The university reviews your documents and usually issues an official invitation or offer letter within 2-4 weeks.",
  },
  {
    title: "Step 5 - Pay the initial registration fee",
    detail:
      "Pay the initial registration or confirmation fee, usually around USD 200-500, to secure the seat and move to the next stage.",
  },
  {
    title: "Step 6 - Apply for the Russian student visa",
    detail:
      "Visit the nearest Russian Embassy or Consulate with your invitation letter, passport, health papers, and financial proof. Processing usually takes 2-4 weeks.",
  },
  {
    title: "Step 7 - Complete pre-departure formalities",
    detail:
      "Get documents attested, apostilled, insured, and travel-ready. Families should also confirm payment channels and winter preparation before departure.",
  },
  {
    title: "Step 8 - Travel and report to university",
    detail:
      "Reach Russia in August, complete university registration, medical checks, and hostel move-in, then begin orientation before the academic session starts.",
  },
];

export const eligibility = [
  {
    Category: "General",
    "NEET Minimum Percentile": "50th percentile",
    "Minimum Marks (Approx.)": "About 137+ marks",
    "PCB Requirement": "50% in Class 12",
    "Minimum Age": "17+ years",
  },
  {
    Category: "SC / ST / OBC",
    "NEET Minimum Percentile": "40th percentile",
    "Minimum Marks (Approx.)": "About 107+ marks",
    "PCB Requirement": "45% in Class 12",
    "Minimum Age": "17+ years",
  },
  {
    Category: "PwD (General)",
    "NEET Minimum Percentile": "45th percentile",
    "Minimum Marks (Approx.)": "About 122+ marks",
    "PCB Requirement": "As per applicable norms",
    "Minimum Age": "17+ years",
  },
];

export const eligibilityNotes = [
  "There is no separate minimum NEET score cutoff imposed by most Russian universities themselves; the NMC percentile rule is the real qualifying bar for Indian students.",
  "Even a NEET score in the 150-200 band can still be enough for admission to many Russian universities if the India-side eligibility requirement is satisfied.",
  "Your passport should ideally have at least 18 months of validity before you begin the visa and invitation process.",
  "Students should verify the university's current NMC and WDOMS status before paying any confirmation fee.",
];

export const universities = [
  { "#": "1", University: "Kazan State Medical University", "Annual Tuition (INR)": "Rs 5.7 lakh", "Annual Hostel (INR)": "Rs 50,000", "Total 6-Year Cost (Approx.)": "Rs 37 lakh" },
  { "#": "2", University: "Kursk State Medical University", "Annual Tuition (INR)": "Rs 4.2 lakh", "Annual Hostel (INR)": "Rs 82,000", "Total 6-Year Cost (Approx.)": "Rs 30 lakh" },
  { "#": "3", University: "Pirogov Russian National Research Medical University", "Annual Tuition (INR)": "Rs 6.1 lakh", "Annual Hostel (INR)": "Rs 90,000", "Total 6-Year Cost (Approx.)": "Rs 42 lakh" },
  { "#": "4", University: "Perm State Medical University", "Annual Tuition (INR)": "Rs 3.7 lakh", "Annual Hostel (INR)": "Rs 98,000", "Total 6-Year Cost (Approx.)": "Rs 28 lakh" },
  { "#": "5", University: "Bashkir State Medical University", "Annual Tuition (INR)": "Rs 2.9 lakh", "Annual Hostel (INR)": "Rs 74,000", "Total 6-Year Cost (Approx.)": "Rs 22 lakh" },
  { "#": "6", University: "Altai State Medical University", "Annual Tuition (INR)": "Rs 3.0 lakh", "Annual Hostel (INR)": "Rs 57,000", "Total 6-Year Cost (Approx.)": "Rs 21 lakh" },
  { "#": "7", University: "Volgograd State Medical University", "Annual Tuition (INR)": "Rs 4.9 lakh", "Annual Hostel (INR)": "Rs 90,000", "Total 6-Year Cost (Approx.)": "Rs 34 lakh" },
  { "#": "8", University: "Northern State Medical University", "Annual Tuition (INR)": "Rs 3.3 lakh", "Annual Hostel (INR)": "Rs 57,000", "Total 6-Year Cost (Approx.)": "Rs 23 lakh" },
  { "#": "9", University: "Kazan Federal University", "Annual Tuition (INR)": "Rs 4.5 lakh", "Annual Hostel (INR)": "Rs 82,000", "Total 6-Year Cost (Approx.)": "Rs 32 lakh" },
  { "#": "10", University: "St. Petersburg State Medical University", "Annual Tuition (INR)": "Rs 4.5 lakh", "Annual Hostel (INR)": "Rs 1.3 lakh", "Total 6-Year Cost (Approx.)": "Rs 34 lakh" },
];

export const feeBreakdown = [
  { University: "Voronezh State Medical University", "Tuition / Year (USD)": "3,500", "Tuition / Year (INR)": "Rs 2,05,000", "Hostel / Year (USD)": "800", "Hostel / Year (INR)": "Rs 65,600" },
  { University: "Dagestan State Medical University", "Tuition / Year (USD)": "3,000", "Tuition / Year (INR)": "Rs 2,46,000", "Hostel / Year (USD)": "1,000", "Hostel / Year (INR)": "Rs 82,000" },
  { University: "Far Eastern Federal University", "Tuition / Year (USD)": "3,000", "Tuition / Year (INR)": "Rs 2,46,000", "Hostel / Year (USD)": "1,000", "Hostel / Year (INR)": "Rs 82,000" },
  { University: "Amur State University (Medical Academy)", "Tuition / Year (USD)": "3,100", "Tuition / Year (INR)": "Rs 2,54,200", "Hostel / Year (USD)": "500", "Hostel / Year (INR)": "Rs 41,000" },
  { University: "Ingush State University", "Tuition / Year (USD)": "2,200", "Tuition / Year (INR)": "Rs 1,80,400", "Hostel / Year (USD)": "550", "Hostel / Year (INR)": "Rs 45,100" },
  { University: "Siberian State Medical University", "Tuition / Year (USD)": "3,000", "Tuition / Year (INR)": "Rs 2,46,000", "Hostel / Year (USD)": "500", "Hostel / Year (INR)": "Rs 41,000" },
  { University: "Mari State University", "Tuition / Year (USD)": "3,200", "Tuition / Year (INR)": "Rs 2,62,400", "Hostel / Year (USD)": "800", "Hostel / Year (INR)": "Rs 65,600" },
  { University: "North Ossetian State University (Medical Academy)", "Tuition / Year (USD)": "3,400", "Tuition / Year (INR)": "Rs 2,78,800", "Hostel / Year (USD)": "400", "Hostel / Year (INR)": "Rs 32,800" },
  { University: "Kuban State Medical University", "Tuition / Year (USD)": "3,500", "Tuition / Year (INR)": "Rs 2,87,000", "Hostel / Year (USD)": "700", "Hostel / Year (INR)": "Rs 57,400" },
  { University: "Bashkir State Medical University", "Tuition / Year (USD)": "3,500", "Tuition / Year (INR)": "Rs 2,87,000", "Hostel / Year (USD)": "900", "Hostel / Year (INR)": "Rs 73,800" },
  { University: "Altai State Medical University", "Tuition / Year (USD)": "3,700", "Tuition / Year (INR)": "Rs 3,03,400", "Hostel / Year (USD)": "500", "Hostel / Year (INR)": "Rs 41,000" },
  { University: "Kadyrov Chechen State University", "Tuition / Year (USD)": "3,700", "Tuition / Year (INR)": "Rs 3,03,400", "Hostel / Year (USD)": "600", "Hostel / Year (INR)": "Rs 49,200" },
  { University: "Pacific State Medical University", "Tuition / Year (USD)": "3,900", "Tuition / Year (INR)": "Rs 3,19,800", "Hostel / Year (USD)": "700", "Hostel / Year (INR)": "Rs 57,400" },
  { University: "Northern State Medical University", "Tuition / Year (USD)": "4,000", "Tuition / Year (INR)": "Rs 3,28,000", "Hostel / Year (USD)": "700", "Hostel / Year (INR)": "Rs 57,400" },
  { University: "Orenburg State Medical University", "Tuition / Year (USD)": "4,000", "Tuition / Year (INR)": "Rs 3,28,000", "Hostel / Year (USD)": "1,000", "Hostel / Year (INR)": "Rs 82,000" },
  { University: "Tambov State University", "Tuition / Year (USD)": "4,200", "Tuition / Year (INR)": "Rs 3,44,400", "Hostel / Year (USD)": "1,200", "Hostel / Year (INR)": "Rs 98,400" },
  { University: "Crimean Federal University", "Tuition / Year (USD)": "4,250", "Tuition / Year (INR)": "Rs 3,48,500", "Hostel / Year (USD)": "600", "Hostel / Year (INR)": "Rs 49,200" },
  { University: "Syktyvkar State Medical University", "Tuition / Year (USD)": "4,250", "Tuition / Year (INR)": "Rs 3,48,500", "Hostel / Year (USD)": "1,200", "Hostel / Year (INR)": "Rs 98,400" },
  { University: "Ryazan State Medical University", "Tuition / Year (USD)": "4,500", "Tuition / Year (INR)": "Rs 3,69,000", "Hostel / Year (USD)": "900", "Hostel / Year (INR)": "Rs 73,800" },
  { University: "Perm State Medical University", "Tuition / Year (USD)": "4,500", "Tuition / Year (INR)": "Rs 3,69,000", "Hostel / Year (USD)": "1,200", "Hostel / Year (INR)": "Rs 98,400" },
  { University: "Novosibirsk State Medical University", "Tuition / Year (USD)": "5,000", "Tuition / Year (INR)": "Rs 4,10,000", "Hostel / Year (USD)": "900", "Hostel / Year (INR)": "Rs 73,800" },
  { University: "Kursk State Medical University", "Tuition / Year (USD)": "5,100", "Tuition / Year (INR)": "Rs 4,18,200", "Hostel / Year (USD)": "1,000", "Hostel / Year (INR)": "Rs 82,000" },
  { University: "Kazan Federal University", "Tuition / Year (USD)": "5,500", "Tuition / Year (INR)": "Rs 4,51,000", "Hostel / Year (USD)": "1,000", "Hostel / Year (INR)": "Rs 82,000" },
  { University: "Krasnoyarsk State Medical University", "Tuition / Year (USD)": "5,500", "Tuition / Year (INR)": "Rs 4,51,000", "Hostel / Year (USD)": "1,200", "Hostel / Year (INR)": "Rs 98,400" },
  { University: "St. Petersburg State Medical University", "Tuition / Year (USD)": "5,500", "Tuition / Year (INR)": "Rs 4,51,000", "Hostel / Year (USD)": "1,600", "Hostel / Year (INR)": "Rs 1,31,200" },
  { University: "Volgograd State Medical University", "Tuition / Year (USD)": "6,000", "Tuition / Year (INR)": "Rs 4,92,000", "Hostel / Year (USD)": "1,100", "Hostel / Year (INR)": "Rs 90,200" },
  { University: "Kazan State Medical University", "Tuition / Year (USD)": "7,000", "Tuition / Year (INR)": "Rs 5,74,000", "Hostel / Year (USD)": "600", "Hostel / Year (INR)": "Rs 49,200" },
  { University: "People's Friendship University of Russia", "Tuition / Year (USD)": "7,000", "Tuition / Year (INR)": "Rs 5,74,000", "Hostel / Year (USD)": "1,200", "Hostel / Year (INR)": "Rs 98,400" },
  { University: "First Moscow State Medical University", "Tuition / Year (USD)": "10,000", "Tuition / Year (INR)": "Rs 8,20,000", "Hostel / Year (USD)": "1,800", "Hostel / Year (INR)": "Rs 1,47,600" },
];

export const additionalCosts = [
  { Expense: "Hostel / accommodation", "Monthly Cost (INR)": "Rs 4,000-Rs 8,000", Notes: "University dorms remain the lowest-friction default for most Indian students." },
  { Expense: "Food (mess + groceries)", "Monthly Cost (INR)": "Rs 5,000-Rs 8,000", Notes: "Indian mess availability depends on city and university ecosystem." },
  { Expense: "Transportation", "Monthly Cost (INR)": "Rs 1,000-Rs 2,000", Notes: "Regional cities are usually cheaper than Moscow or St. Petersburg." },
  { Expense: "Personal expenses", "Monthly Cost (INR)": "Rs 2,000-Rs 4,000", Notes: "This usually covers basic daily use, clothing, and minor purchases." },
  { Expense: "Internet and phone", "Monthly Cost (INR)": "Rs 500-Rs 1,000", Notes: "Most students maintain one local SIM plus hostel or apartment Wi-Fi." },
  { Expense: "Total monthly estimate", "Monthly Cost (INR)": "Rs 12,500-Rs 23,000", Notes: "This is the broad working band families should budget around in 2026." },
];

export const fmgeContext = [
  { Year: "2022", Appeared: "6,069", Passed: "1,546", "Pass Rate": "25.5%" },
  { Year: "2023", Appeared: "About 9,500", Passed: "About 2,660", "Pass Rate": "28.0%" },
  { Year: "2024", Appeared: "11,276", Passed: "3,331", "Pass Rate": "29.54%" },
];

export const universityFmge = [
  { "FMGE Insight": "Top-ranked NMC-approved Russian universities", "Why It Matters": "Commonly discussed in the 40-50% FMGE band when the university quality, language model, and student preparation are stronger." },
  { "FMGE Insight": "Mid-tier mainstream options", "Why It Matters": "Often sit around the broader Russia average, which means serious NExT or FMGE preparation becomes even more important." },
  { "FMGE Insight": "Lower-ranked fee-only options", "Why It Matters": "These can fall into the 10-15% outcome band, which is why fee-only shortlisting is risky." },
  { "FMGE Insight": "Student strategy", "Why It Matters": "Starting FMGE or NExT preparation from Year 4, not after graduation, materially improves the odds of a stronger India-return outcome." },
];

export const recognition = [
  { Body: "NMC", FullName: "National Medical Commission", "Why It Matters": "Required for the India licensing pathway after graduation." },
  { Body: "WHO", FullName: "World Health Organization", "Why It Matters": "Confirms the global legitimacy of the university listing ecosystem." },
  { Body: "FAIMER", FullName: "Foundation for Advancement of International Medical Education", "Why It Matters": "Supports broader international verification and education visibility." },
  { Body: "ECFMG", FullName: "Educational Commission for Foreign Medical Graduates", "Why It Matters": "Important for later USA pathway planning through USMLE and related checks." },
  { Body: "WFME", FullName: "World Federation for Medical Education", "Why It Matters": "Useful in the wider recognition and quality-assurance ecosystem." },
  { Body: "Ministry of Education, Russia", FullName: "Government-approved university status", "Why It Matters": "Confirms the university is recognised in Russia before India-side recognition is even considered." },
];

export const syllabus = [
  { Year: "1st Year", "Semester 1": "Biology, Anatomy, Russian Language, Latin, Physical Education, Nursing", "Semester 2": "Physics, Russian Language, Biology, Latin, Nursing, Anatomy, Histology" },
  { Year: "2nd Year", "Semester 1": "Russian Language, Anatomy, Histology, Physiology, Biochemistry", "Semester 2": "Russian Language, Physiology, Microbiology, Immunology, Biochemistry" },
  { Year: "3rd Year", "Semester 1": "Microbiology, Immunology, Radiology, Surgery, Pharmacology, Pathophysiology", "Semester 2": "Pathoanatomy, Internal Medicine, Surgery, Pharmacology, Pathophysiology, Topo Anatomy" },
  { Year: "4th Year", "Semester 1": "Topo Anatomy, Hygiene, Oncology, Gynecology, Neurology", "Semester 2": "Gynecology, Neurology, Ophthalmology, Psychiatry, Pediatrics" },
  { Year: "5th Year", "Semester 1": "Therapy, Pediatrics, Psychiatry, Obstetrics, Hospital Surgery, Dermatology", "Semester 2": "Clinical Anatomy, Traumatology, Obstetrics, Traumatology, Therapy, Hospital Surgery" },
  { Year: "6th Year", "Semester 1": "Obstetrics, Gynecology, Therapy, Lab Diagnostics", "Semester 2": "Surgery, Pediatrics, Emergency Medicine" },
];

export const gozzSteps = [
  {
    title: "Written test",
    detail:
      "A multiple-choice examination covering the core medical subjects studied during the degree.",
  },
  {
    title: "Practical assessment",
    detail:
      "Real patient examination, case work, and clinical reporting are assessed in a practical format.",
  },
  {
    title: "Oral examination",
    detail:
      "Students discuss clinical cases and reasoning in a viva-style format before receiving the final degree outcome.",
  },
];

export const licensingSteps = [
  "Clear the GOZZ examination and complete all university graduation requirements in Russia.",
  "Receive the Russian MBBS-equivalent degree and local licensing documents issued after graduation.",
  "Apostille and organise the degree, transcript, and supporting academic records for India-side processing.",
  "Return to India and prepare for the current NExT or FMGE-linked licensing pathway applicable at that time.",
  "Clear the Indian licensing requirements and complete the required registration steps with the appropriate authority.",
  "Proceed into general practice, PG entrance, or international pathways once your India or foreign licensing path is active.",
];

export const livingCosts = [
  { Expense: "Hostel / accommodation", "Monthly Cost (INR)": "Rs 4,000-Rs 8,000" },
  { Expense: "Food (mess + groceries)", "Monthly Cost (INR)": "Rs 5,000-Rs 8,000" },
  { Expense: "Transportation", "Monthly Cost (INR)": "Rs 1,000-Rs 2,000" },
  { Expense: "Personal expenses", "Monthly Cost (INR)": "Rs 2,000-Rs 4,000" },
  { Expense: "Internet and phone", "Monthly Cost (INR)": "Rs 500-Rs 1,000" },
  { Expense: "Total monthly estimate", "Monthly Cost (INR)": "Rs 12,500-Rs 23,000" },
];

export const vacationBreaks = [
  {
    title: "Summer vacation",
    detail:
      "Usually July-August, around 2 months. Most Indian students use this period to return home, and the new academic year typically restarts on 1 September.",
  },
  {
    title: "Winter vacation",
    detail:
      "Usually from mid-January to the first week of February, giving students around 3 weeks of winter break.",
  },
  {
    title: "Flight planning note",
    detail:
      "Round-trip flights for summer return planning commonly fall in the Rs 35,000-Rs 55,000 range depending on city and booking window.",
  },
];

export const accommodationNotes = [
  "Most university hostels in the major Indian-student cities offer access to Indian vegetarian and non-vegetarian meals nearby or through Indian mess support.",
  "Indian restaurants and regional food options are easier to find in cities like Moscow, Kazan, Kursk, and St. Petersburg than in smaller regional cities.",
  "Shared apartments usually cost around Rs 7,000-Rs 12,000 per month, while university hostels often stay in the Rs 4,000-Rs 8,000 band.",
  "Indian grocery access is strongest in cities with a larger Indian student base, while smaller cities often require more self-cooking and advance planning.",
];

export const advantages = [
  "Affordable total cost compared with Indian private colleges and many European destinations.",
  "No donation or capitation fee in the standard route; the process is documentation-based and transparent.",
  "NMC and WHO-approved degrees are available at many long-established Russian medical universities.",
  "English-medium programs are available at most of the better-known universities.",
  "Clinical exposure usually becomes more meaningful from Year 3 onward in the larger university hospital ecosystems.",
  "A large Indian student community exists in several established Russian medical cities.",
  "The admission process is usually simple because no university-specific entrance exam is required beyond NEET for Indian eligibility.",
];

export const disadvantages = [
  "The overall FMGE pass rate is about 29.5%, so dedicated preparation is non-negotiable for India return.",
  "Basic Russian is still important for patient interaction and daily life even in English-medium programs.",
  "Winter temperatures can become severe in colder cities, especially beyond western Russia.",
  "Not every low-fee Russian university is equally safe for India-return outcomes, so shortlisting mistakes are costly.",
  "Currency fluctuations and payment-channel changes can affect year-by-year budgeting.",
  "International scholarships are limited compared with some other study-abroad routes.",
];

export const comparison = [
  { Country: "Russia", "Total Cost": "Rs 20 lakh-Rs 38 lakh", "NMC Seats": "55+", "FMGE Pass Rate": "About 29.5%", Language: "English + Russian basics" },
  { Country: "Uzbekistan", "Total Cost": "Rs 18 lakh-Rs 28 lakh", "NMC Seats": "15+", "FMGE Pass Rate": "About 25%", Language: "English" },
  { Country: "Philippines", "Total Cost": "Rs 25 lakh-Rs 40 lakh", "NMC Seats": "30+", "FMGE Pass Rate": "About 25-30%", Language: "English" },
  { Country: "Kazakhstan", "Total Cost": "Rs 20 lakh-Rs 30 lakh", "NMC Seats": "20+", "FMGE Pass Rate": "40-55% (Al-Farabi 60-65%)", Language: "English / Russian" },
  { Country: "Georgia", "Total Cost": "Rs 28 lakh-Rs 42 lakh", "NMC Seats": "15+", "FMGE Pass Rate": "65-75% at top universities (DTMU, TSMU)", Language: "English" },
  { Country: "Germany", "Total Cost": "Government-funded route", "NMC Seats": "5+", "FMGE Pass Rate": "Not applicable", Language: "German required" },
];

export const scholarships = [
  { "Scholarship Type": "Russian Government Scholarship", Coverage: "Full tuition + stipend + accommodation", "How to Apply": "Apply via the Indian Embassy / Russian government education route" },
  { "Scholarship Type": "University Merit Scholarships", Coverage: "Partial tuition waiver", "How to Apply": "Apply directly to the university" },
  { "Scholarship Type": "NSP (National Scholarship Portal)", Coverage: "Indian government scholarship support", "How to Apply": "Apply via nsp.gov.in where applicable" },
  { "Scholarship Type": "Education Loans (SBI / Bank of Baroda and other public lenders)", Coverage: "Up to about Rs 20 lakh-Rs 40 lakh depending on profile", "How to Apply": "Apply at the bank branch with the admission letter and supporting documents" },
];

export const documents = [
  "Class 10 and 12 mark sheets and certificates",
  "NEET 2026 scorecard",
  "Valid passport with at least 18 months validity",
  "Passport-size photographs (minimum 8)",
  "Birth certificate",
  "Recent HIV test report",
  "Medical fitness certificate",
  "Bank statement / financial proof",
  "No-objection certificate where required by the processing route",
  "Health insurance and travel insurance certificates",
  "Invitation letter from the university after acceptance",
  "Attestation from the Ministry of External Affairs (MEA)",
  "Legalisation or embassy-side completion where required for the visa route",
];

export const careerPathways = [
  { Pathway: "Practice in India", Country: "India", "Exam Required": "FMGE / NExT" },
  { Pathway: "MD / MS specialisation in India", Country: "India", "Exam Required": "NEET-PG" },
  { Pathway: "US residency pathway", Country: "USA", "Exam Required": "USMLE Step 1, 2, 3" },
  { Pathway: "Practice in the UK", Country: "United Kingdom", "Exam Required": "PLAB 1 and 2" },
  { Pathway: "PG in Germany (paid residency)", Country: "Germany", "Exam Required": "FSP + German B2/C1 pathway" },
  { Pathway: "Practice in Australia", Country: "Australia", "Exam Required": "AMC exam" },
  { Pathway: "PG residency in Russia", Country: "Russia", "Exam Required": "GOZZ + specialisation route" },
];

export const faqs = [
  {
    question: "Is NEET required for MBBS in Russia?",
    answer:
      "Yes. As per NMC's 2021 regulations, NEET is mandatory for all Indian students applying to foreign medical universities, including Russia. The minimum qualifying percentile is 50th for General and 40th for SC, ST, and OBC.",
  },
  {
    question: "What is the total cost of MBBS in Russia for an Indian student?",
    answer:
      "Total 6-year cost including tuition, hostel, food, travel, and personal expenses ranges from Rs 22 lakh-Rs 40 lakh depending on the university and city. Budget universities like Bashkir State and Altai State usually stay around Rs 21 lakh-Rs 23 lakh all-in, while premium options like Pirogov RNRMU or First Moscow State can move toward the Rs 40 lakh-Rs 50 lakh band.",
  },
  {
    question: "Is MBBS from Russia valid in India?",
    answer:
      "Yes. If you graduate from an NMC-approved Russian university and clear the NExT licensing route, you can practise medicine in India. Always verify your university's current approval status at wdoms.org before paying any fee.",
  },
  {
    question: "What is the FMGE pass rate for students from Russia?",
    answer:
      "The overall FMGE pass rate for Indian students from Russian universities is 29.54% in the 2024 NMC data. Top universities like Kazan State, Kursk State, and Perm State are commonly discussed in the 40-50% band when paired with serious student preparation from Year 4 onward.",
  },
  {
    question: "Which is the best Russian university for Indian students?",
    answer:
      "Kazan State Medical University, Kursk State Medical University, and Perm State Medical University are strong India-return focused choices. For budget-conscious families, Bashkir State Medical University and Altai State Medical University remain popular value picks.",
  },
  {
    question: "When does MBBS admission in Russia start in 2026?",
    answer:
      "Applications usually open January-March 2026. Invitation letters are generally issued March-April, visa processing runs June-August, and the academic session begins September-October 2026. Students should ideally begin research in November-December 2025.",
  },
  {
    question: "How many years is MBBS in Russia?",
    answer:
      "MBBS in Russia is usually structured as a 6-year program: 5 years of academic study plus 1 year of clinical internship or integrated clinical training depending on the university model.",
  },
  {
    question: "Can I practise in India after MBBS from Russia?",
    answer:
      "Yes. After graduation, you return to India, clear the required licensing route such as FMGE or NExT as applicable, and then complete the Indian registration process before practice.",
  },
  {
    question: "Is there any entrance exam for MBBS in Russia?",
    answer:
      "No separate university-specific entrance exam is usually required. For Indian students, the main mandatory exam is NEET, and admission is otherwise based on documents and academic profile.",
  },
  {
    question: "Can I do a part-time job in Russia while studying MBBS?",
    answer:
      "Russian student-visa rules are more restrictive than many students assume, so students should not build their budget around external part-time work. University-side assistantships or small institutional roles are a more realistic possibility than outside employment.",
  },
  {
    question: "What is the medium of instruction for MBBS in Russia?",
    answer:
      "The top Russian medical universities offer English-medium programs, but most students still learn basic Russian from Year 1 because clinical patient interaction requires it later.",
  },
  {
    question: "How is clinical exposure in Russia compared with other countries?",
    answer:
      "Russia offers meaningful clinical training from around Year 3 onward, especially in the larger government hospital ecosystems attached to the stronger medical universities. Patient volumes are usually high, which helps practical exposure.",
  },
  {
    question: "Are there coaching programs for FMGE or NExT preparation in Russia?",
    answer:
      "Yes. Many Indian student communities, online platforms, and selected local mentorship setups help students prepare for FMGE or NExT while they are still in Russia. Starting from Year 4 is usually a better strategy than waiting until graduation.",
  },
  {
    question: "What are the PG options after MBBS in Russia?",
    answer:
      "Common routes include MD or MS in India through NEET-PG, the USMLE route for USA residency, PLAB for the UK, Germany's doctor pathway through German language plus FSP, AMC for Australia, or PG specialisation inside Russia itself.",
  },
  {
    question: "What is the weather like in Russia for Indian students?",
    answer:
      "Russian winters can be severe. Cities like Novosibirsk or Orenburg can reach minus 15 degrees Celsius to minus 30 degrees Celsius in winter, while places like Kazan, Kursk, and Volgograd are usually milder. Families should budget around Rs 15,000-Rs 25,000 for a proper first-winter clothing setup.",
  },
];

export const articleSchema = {
  "@context": "https://schema.org",
  "@type": "Article",
  headline: pageTitle,
  description: metaDescription,
  datePublished: "2026-03-01",
  dateModified: "2026-03-01",
  author: {
    "@type": "Organization",
    name: "MBBS Admissions in Abroad",
  },
  publisher: {
    "@type": "Organization",
    name: "MBBS Admissions in Abroad",
  },
  mainEntityOfPage: pageUrl,
};

export const breadcrumbSchema = {
  "@context": "https://schema.org",
  "@type": "BreadcrumbList",
  itemListElement: [
    { "@type": "ListItem", position: 1, name: "Home", item: siteUrl },
    { "@type": "ListItem", position: 2, name: "MBBS Abroad", item: `${siteUrl}/mbbs-admission-in-europe-from-mci-approved-university` },
    { "@type": "ListItem", position: 3, name: "MBBS in Russia", item: pageUrl },
  ],
};

export const faqSchema = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  mainEntity: faqs.map((faq) => ({
    "@type": "Question",
    name: faq.question,
    acceptedAnswer: {
      "@type": "Answer",
      text: faq.answer,
    },
  })),
};
