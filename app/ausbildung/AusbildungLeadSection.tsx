...
import { ivrNumber, topCourses, whatsappHref, whatsappNumber } from "./pageData";
...
function AusbildungLeadForm({
  ...
  const courseOptions = useMemo(() => topCourses.map((item) => item["Course (German Name)"]), []);

  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label htmlFor="ausbildung-name" className="mb-1 block text-sm font-medium text-slate-700">
          Full name
        </label>
        <input
          id="ausbildung-name"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label htmlFor="ausbildung-phone" className="mb-1 block text-sm font-medium text-slate-700">
            WhatsApp number
          </label>
          <input
            id="ausbildung-phone"
        ...
        <div>
          <label htmlFor="ausbildung-email" className="mb-1 block text-sm font-medium text-slate-700">
            Email
          </label>
          <input
            id="ausbildung-email"
      ...
      <div>
        <label htmlFor="ausbildung-course" className="mb-1 block text-sm font-medium text-slate-700">
          Preferred Ausbildung field
        </label>
        <select
          id="ausbildung-course"
...
export default function AusbildungLeadSection() {
  ...
  const submitLead = async (
    ...
    const requestData = {
      fields: {
        ...
        College: activeForm.preferredCourse,
      },
      actions: [
        {
          type: "SYSTEM_NOTE",
          text: "Lead Source: Ausbildung in Germany Page",
        },
      ],
    };
    ...
    try {
      ...
      router.push("/thank-you?title=Ausbildung%20in%20Germany");
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      {showPopup ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[32px] bg-white p-6 shadow-2xl md:p-8">
            ...
            <h3 className="mt-3 text-3xl font-bold text-slate-950">Get personalised Ausbildung guidance</h3>
            <p className="mt-3 text-base leading-7 text-slate-600">Share your details and our team can help with field selection, language planning, company applications, visa readiness and whether Ausbildung fits your goals better than a university route.</p>
      ...
      <section id="ausbildung-contact-form" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[32px] bg-[linear-gradient(135deg,#1d4ed8_0%,#0f172a_100%)] p-8 text-white">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-200">Contact Ausbildung Desk</p>
            <h2 className="mt-4 text-3xl font-bold">Speak to our team on WhatsApp or request a callback</h2>
            <p className="mt-4 text-base leading-8 text-slate-300">Use this section for field shortlisting, German-language planning, company application support and 2026 visa guidance.</p>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
            ...
            <h3 className="mt-4 text-3xl font-bold text-slate-950">Request Ausbildung support</h3>
            <p className="mt-3 text-base leading-7 text-slate-600">Fill this once and the team can contact you with realistic field options, language requirements and the documents you should prepare first.</p>
            <div className="mt-8">
              <AusbildungLeadForm
                formData={formData}
