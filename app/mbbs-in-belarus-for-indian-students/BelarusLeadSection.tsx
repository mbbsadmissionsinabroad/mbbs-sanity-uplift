...
function BelarusLeadForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label htmlFor="belarus-name" className="mb-1 block text-sm font-medium text-slate-700">
          Full name
        </label>
        <input
          id="belarus-name"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label htmlFor="belarus-phone" className="mb-1 block text-sm font-medium text-slate-700">
            WhatsApp number
          </label>
          <input
            id="belarus-phone"
        ...
        <div>
          <label htmlFor="belarus-email" className="mb-1 block text-sm font-medium text-slate-700">
            Email
          </label>
          <input
            id="belarus-email"
      ...
      <div>
        <label htmlFor="belarus-university" className="mb-1 block text-sm font-medium text-slate-700">
          Preferred university
        </label>
        <select
          id="belarus-university"
...
export default function BelarusLeadSection() {
  ...
  const submitLead = async (
    ...
    const requestData = {
      fields: {
        ...
        Course: "MBBS Abroad",
        StudyCountry: "Belarus",
        College: activeForm.preferredUniversity,
      },
      actions: [
        {
          type: "SYSTEM_NOTE",
          text: "Lead Source: MBBS in Belarus Page",
        },
      ],
    };
    ...
    try {
      ...
      router.push("/thank-you?title=MBBS%20in%20Belarus");
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      {showPopup ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[32px] bg-white p-6 shadow-2xl md:p-8">
            ...
            <h3 className="mt-3 text-3xl font-bold text-slate-950">Get personalised Belarus admission guidance</h3>
            <p className="mt-3 text-base leading-7 text-slate-600">Share your details and our team can help with university shortlisting, budget planning, India-return strategy and whether Belarus is the right low-cost route for you.</p>
      ...
      <section id="belarus-contact-form" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[32px] bg-[linear-gradient(135deg,#1d4ed8_0%,#0f172a_100%)] p-8 text-white">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-200">Contact Belarus Desk</p>
            <h2 className="mt-4 text-3xl font-bold">Speak to our team on WhatsApp or request a callback</h2>
            <p className="mt-4 text-base leading-8 text-slate-300">Use this section for Belarus university comparison, FMGE-focused planning, budget discussion and 2026-27 intake support.</p>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
            ...
            <p className="mt-3 text-base leading-7 text-slate-600">Fill this once and the team can contact you with Belarus options that fit your score, budget and India-return plans.</p>
            <div className="mt-8">
              <BelarusLeadForm
                formData={formData}
