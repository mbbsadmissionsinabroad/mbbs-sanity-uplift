...
function CanadaNursingLeadForm({
  formData,
  onChange,
  onSubmit,
  isLoading,
  ...
  const provinceOptions = useMemo(
    () => provinces.map((item) => item["Province / Employer"]),
    []
  );
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label htmlFor="canada-nursing-name" className="mb-1 block text-sm font-medium text-slate-700">
          Full name
        </label>
        <input
          id="canada-nursing-name"
          name="name"
          value={formData.name}
          onChange={onChange}
          required
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label htmlFor="canada-nursing-phone" className="mb-1 block text-sm font-medium text-slate-700">
            WhatsApp number
          </label>
          <input
            id="canada-nursing-phone"
            name="phone"
        ...
        <div>
          <label htmlFor="canada-nursing-email" className="mb-1 block text-sm font-medium text-slate-700">
            Email
          </label>
          <input
            id="canada-nursing-email"
            type="email"
            name="email"
      ...
      <div>
        <label htmlFor="canada-nursing-province" className="mb-1 block text-sm font-medium text-slate-700">
          Preferred province
        </label>
        <select
          id="canada-nursing-province"
...
export default function CanadaNursingLeadSection() {
  ...
  const submitLead = async (
    ...
    const requestData = {
      fields: {
        ...
        StudyCountry: "Canada",
        College: activeForm.preferredProvince,
      },
      actions: [
        {
          type: "SYSTEM_NOTE",
          text: "Lead Source: Nursing Jobs in Canada Page",
        },
      ],
    };
    ...
    try {
      ...
      router.push("/thank-you?title=Nursing%20Jobs%20in%20Canada");
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[32px] bg-white p-6 shadow-2xl md:p-8">
            ...
            <h3 className="mt-3 text-3xl font-bold text-slate-950">
              Get personalised Canada nursing guidance
            </h3>
            <p className="mt-3 text-base leading-7 text-slate-600">
              Share your details and our team will help with eligibility, NNAS planning, NCLEX preparation, province selection and PR strategy.
            </p>
      ...
      <section id="canada-nursing-contact-form" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
            ...
            <h3 className="mt-4 text-3xl font-bold text-slate-950">
              Request Canada nursing support
            </h3>
            <p className="mt-3 text-base leading-7 text-slate-600">
