...
function NetherlandsLeadForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label htmlFor="netherlands-name" className="mb-1 block text-sm font-medium text-slate-700">
          Full name
        </label>
        <input
          id="netherlands-name"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label htmlFor="netherlands-phone" className="mb-1 block text-sm font-medium text-slate-700">
            WhatsApp number
          </label>
          <input
            id="netherlands-phone"
            name="phone"
        ...
        <div>
          <label htmlFor="netherlands-email" className="mb-1 block text-sm font-medium text-slate-700">
            Email
          </label>
          <input
            id="netherlands-email"
            type="email"
            name="email"
      ...
      <div>
        <label htmlFor="netherlands-university" className="mb-1 block text-sm font-medium text-slate-700">
          Preferred university
        </label>
        <select
          id="netherlands-university"
...
export default function NetherlandsLeadSection() {
  ...
  const submitLead = async (
    ...
    const requestData = {
      ...
      actions: [
        {
          type: "SYSTEM_NOTE",
          text: "Lead Source: MBBS in Netherlands Page",
        },
      ],
    };
    ...
    try {
      ...
      router.push("/thank-you?title=MBBS%20in%20Netherlands");
    } catch (error) {
      console.error(error);
  ...
  return (
    <>
      ...
      {showPopup ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[32px] bg-white p-6 shadow-2xl md:p-8">
            ...
            <h3 className="mt-3 text-3xl font-bold text-slate-950">Get personalised Netherlands admission guidance</h3>
            <p className="mt-3 text-base leading-7 text-slate-600">Share your details and our team can help with Dutch language planning, university shortlisting, budget review and whether the Netherlands actually fits your long-term goals.</p>
      ...
      <section id="netherlands-contact-form" className="mx-auto max-w-7xl px-4 py-16 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-[32px] bg-[linear-gradient(135deg,#1d4ed8_0%,#0f172a_100%)] p-8 text-white">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-200">Contact Netherlands Desk</p>
            <h2 className="mt-4 text-3xl font-bold">Speak to our team on WhatsApp or request a callback</h2>
            <p className="mt-4 text-base leading-8 text-slate-300">Use this section for Dutch language planning, university comparison, budget discussion and long-term EU career guidance.</p>
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
            ...
            <p className="mt-3 text-base leading-7 text-slate-600">Fill this once and the team can contact you with Netherlands options that fit your budget, language readiness and long-term plan.</p>
            <div className="mt-8">
              <NetherlandsLeadForm
                formData={formData}
